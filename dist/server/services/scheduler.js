...
export class SchedulerService {
    ...
    async sendReminderMessage(employee) {
        const settings = await storage.getSettings();
        if (!settings)
            return;
        const contacts = await storage.getContacts();
        ...
        console.log(`⏰ Enviando lembrete para ${employee.name} aos contatos:`, activeContacts.map(c => `${c.name} (${c.phone})`));
    ...
    async sendBirthdayMessage(employee) {
        const settings = await storage.getSettings();
        if (!settings)
            return;
        const contacts = await storage.getContacts();
        ...
        console.log(`🎂 Enviando mensagem de aniversário para ${employee.name} aos contatos:`, activeContacts.map(c => `${c.name} (${c.phone})`));
    ...
    async sendMessageWithRetry(employee, contact, message, type, settings) {
        let success = false;
        let attempts = 0;
        let errorMessage = '';
    ...
    formatMessage(template, employee) {
